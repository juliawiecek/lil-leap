 ...
 * routes such as registration, login and refresh keep working; Spring Security's
 * own authorization rules reject unauthenticated requests to protected routes
 * downstream. A header that <em>is</em> present but fails validation is rejected
 * here directly with a 401, since that always indicates a bad or expired token
 * rather than an anonymous request.</p>
 */
